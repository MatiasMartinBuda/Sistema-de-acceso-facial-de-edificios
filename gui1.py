...
def mostrar_puerta_abierta(parent, nombre_persona="", subtitulo="", duracion_ms=None):
    ...
    abriéndose y un cartel de bienvenida personalizado, cada vez que se
    concede un acceso (Caminos A, B o C)."""
    ...
    try:
        img = _generar_imagen_bienvenida(nombre_persona, subtitulo)
        _imagen_puerta_cache = ImageTk.PhotoImage(img)  # referencia fuerte, evita el garbage collector
        tk.Label(top, image=_imagen_puerta_cache, bg=COLOR_FONDO).pack()
    except Exception as e:
        print(f"[Puerta] No se pudo generar la imagen de bienvenida: {e}")
        tk.Label(top, text="🚪", font=("Segoe UI", 60), bg=COLOR_FONDO).pack(padx=40, pady=40)
        tk.Label(top, text=f"✅ ACCESO CONCEDIDO - {nombre_persona}", font=("Segoe UI", 14, "bold"),
                 fg=COLOR_TITULO, bg=COLOR_FONDO).pack(pady=(10, 12))

    boton_estilizado(top, "Cerrar", top.destroy, bg="#555", width=10).pack(pady=8)
    ...
    ms = duracion_ms if duracion_ms is not None else int(settings.get("DURACION_PUERTA_MS"))
    top.after(ms, lambda: top.destroy() if top.winfo_exists() else None)
